...
/**
 * <p>
 * Function： 用于跨组件获取Fragment，如：MainModule中获取news,user,study,interviews中的fragment
 * <p>
 * ver     date      		author
 * ──────────────────────────────────
 * V1.0   2018年03月24日11:53  lin_xiao_zhang@163.com
 * <p>
        ...
        try {
            ...
            if (isYunOS()) {
                ...
                if (versionString != null) {
                    final String TEMP="(\\d+)\\.(\\d+)(\\.\\d+)?";
                    Matcher matcher = Pattern.compile(TEMP).matcher(versionString);
                    if (matcher.matches()) {
                        try {
